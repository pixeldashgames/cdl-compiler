    ...
    def __call__(self, w: List[Token]):
        ...
        while True:
            ...
            try:
                ...
                # actions
                actions = {(k[1], type(k[1])) for k in self.action if k[0] == state}
                raise Exception(
                    f"PARSER ERROR: Invalid token on cursor {cursor} and lookahead {lookahead} ({type(lookahead)}), expected one of {actions}")
