...
def partition_index(state, partition):
    for i in range(len(partition.groups)):
        if state in [n.value for n in partition.groups[i]]:
            return i
...
def state_minimization(automaton):
    ...
    while True:
        ...
        for group in partition.groups:
            sub_groups = distinguish_states([n.value for n in group], automaton, partition)
...
def automata_minimization(automaton):
    ...
    for i, state in enumerate(states):
        ...
        for symbol, destinations in automaton.transitions[origin].items():
            ...
            for e in range(len(states)):
                if dest in [n.value for n in partition.groups[e]]:
                    dest = e
                    break
    ...
    finals = [i for i in range(len(states)) 
              if any([node.value in automaton.finals for node in partition.groups[i]])]
    
    start = None
    
    for i in range(len(states)):
        if automaton.start in [node.value for node in partition.groups[i]]:
            start = i
